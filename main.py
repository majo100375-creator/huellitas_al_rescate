import streamlit as st  #Importamos bibliotecas
import pandas as pd
import numpy as np
import pickle
import json
...
#Red neuronal para analizar las fotos
def analizar_foto_cnn(imagen_pil):
    ...
    if brillo_promedio > 220:
        ...
        return "OSCURA", "Nuestro amigo anda por ahí 👀, no logro verlo esta muy oscuro."   #Posibles respuestas
    else:
...
def cargar_modelos():
    ...
    archivos = {
        ...
        'regresion': 'modelo_regresion.pkl',                 #Carga de modelos entrenados
...
def evaluar_regresion(datos, modelo):
    if modelo is None:
        ...
        if datos['adaptacion'] == "Menos de una semana": score -= 20          #Prediccion por respuestas del usuario
...
def main():
    ...
    if st.session_state.menu_actual == "Inicio":
        ...
        categorias = ["Todos", "Perro", "Gato", "Aves", "Bovinos"]   #División por categorías
                            ...
                            with st.container(border=True):
                                ...
                                for ruta in posibles_rutas:
                                    if os.path.exists(ruta):
                                        img_final = Image.open(ruta)      #Carga de fotos
                                        break
        ...
        with st.form("form_oficial"):
            ...
            with c2: 
                direccion = st.text_input("Dirección")                 #Relleno de formulario
            ...
            if st.form_submit_button("Enviar Solicitud"):
                if aceptar and firma:
                    ...
                    score = evaluar_regresion(datos, modelos['regresion']) #Enviar solicitud desglosa el apartado de predicción de probabilidad
                    if score > 85: 
        ...
        subido = st.file_uploader("Sube la foto...", type=["jpg", "jpeg", "png"])   #Permisión de cualquier formato de imagen
        if subido:
            img = Image.open(subido)
            st.image(img, width=400)
            est, msg = analizar_foto_cnn(img)
            if est == "PERFECTA": st.success(msg)   #Análisis acorde a la base de datos sobre iluminación
        ...
        if animales:
            ...
            df_stats['Presencia Niños'] = np.random.choice(['Si', 'No'], len(df_stats))  #Evaluación de probabilidades
        ...
        with col_form:
            ...
            with st.container(border=True):
                esp = st.selectbox("Especie", ["Dog", "Cat", "Bird", "Livestock"], key="staff_esp")   #Evaluación acorde a base de datos sobre la estancia de una nueva mascota rescatada y sus variables.
            ...
            if calcular:
                with resultado_cont:
                    ...
                    if motivo == "Enfermo (Sick)":
                        st.error("🚨 **Protocolo Médico:** Priorizar casa de acogida para evitar contagios y estrés.")   #Protocolos para variables
                    if edad_m > 95:
                        st.warning("👴 **Perfil Senior:** Se recomienda 'Senior for Senior' (descuento en tasas).")
            
                    st.info("📸**Tip de Marketing:** Asegura una foto de alta calidad en la pestaña 'Análisis de Imágenes'. 🐾Asi aumentaremos las oportunidades de acogida de nuestro amigo peludo🐾.") #Respuestas
